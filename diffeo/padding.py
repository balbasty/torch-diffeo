import torch
from .utils import meshgrid_ij, ensure_list, prod, sub2ind_list
from . import bounds
...
def pad(inp, padsize, mode='constant', value=0, side=None):
    ...
    !!! note
        ...
        twice as many values should be provided, indicating different
        padding sizes for the 'pre' and 'post' sides. If the number of
        padding values is less than the dimension of the input tensor,
        zeros are prepended.
    ...
    mode : {'constant', 'replicate', 'reflect1', 'reflect2', 'circular'}, default='zero'
        Padding mode
    ...
    mode = list(map(lambda x: bounds.bound2dft.get(x, x), mode))
    if all(m == 'zero' for m in mode):
        return _pad_constant(inp, padpre, padpost, value)
    else:
        mode = [getattr(bounds, m + '_') for m in mode]
        return _pad_bound(inp, padpre, padpost, mode)
